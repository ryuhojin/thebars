...
- 시스템 관리자 fixture: `admin1` / `AdminPass!1`
- 바 사용자 fixture: `staff1` / `StaffPass!1`
- 강제 비밀번호 변경 fixture: `forced1` / `TempPass!1`
...
- 시스템 관리자: `admin1` / `AdminPass!1`
- 바 사용자: `staff1` / `StaffPass!1`
...
- 시스템 관리자: `admin1` / `AdminPass!1`
- 바 owner: `owner1` / `OwnerPass!1`
- 바 manager: `manager1` / `ManagerPass!1`
- 바 staff: `staff1` / `StaffPass!1`
...
D15는 `/api/bars/{barId}/publications`에서 현재 저장본을 public JSON으로 변환·검증한 뒤 fake GitHub adapter에 기록한다. 실제 production GitHub token, remote push, production deploy는 사용하지 않는다. 최초 또는 내용 변경 발행은 `public/menus/{encodedSlug}.json`을 쓰고, 동일 내용 재발행은 `public/publish-triggers/{encodedSlug}.json`만 갱신한다. 바별 발행 lock과 고객 repo 전역 commit lock을 분리하며, schema 검증 실패는 GitHub 쓰기 없이 422로 종료한다. 발행 화면은 WF-18의 같은 `/bars/{barId}/publications` URL에서 확인 panel, 진행 단계, 이력, 스냅샷 상세를 compact·medium·wide layout으로만 전환하고 확인 상태와 선택 이력은 resize 후 유지된다. Cloudflare deployment polling과 이력 심화는 D16 범위다.
...
- 초기화: `POST http://127.0.0.1:5173/__dev/reset-auth?fixtures=full`
- 바 manager: `manager1` / `ManagerPass!1` — 기타 항목과 금액 조정 action 표시
- 바 staff: `staff1` / `StaffPass!1` — 주문 운영은 가능하지만 기타 항목과 금액 조정 action 숨김
...
- 초기화: `POST http://127.0.0.1:5173/__dev/reset-auth?fixtures=full`
- 바 manager: `manager1` / `ManagerPass!1` — 주문 라인, 기타 항목, 금액 조정, 계산 요청, 정산, 취소 action 표시
- 바 staff: `staff1` / `StaffPass!1` — 주문 운영과 정산은 가능하지만 기타 항목과 금액 조정 action 숨김
...
- 초기화: `POST http://127.0.0.1:5173/__dev/reset-auth?fixtures=full`
- 시스템 관리자: `admin1` / `AdminPass!1` — 감사 로그와 보관 작업 접근 가능
- 바 manager: `manager1` / `ManagerPass!1` — 일반 운영 화면은 권한에 따라 접근, 감사 로그 접근 불가
- 바 staff: `staff1` / `StaffPass!1` — sidebar에 감사 로그 메뉴 없음
