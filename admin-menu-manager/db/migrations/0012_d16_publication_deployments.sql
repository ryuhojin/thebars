...
CREATE TABLE publications_d16_new (
  id TEXT PRIMARY KEY,
...
INSERT INTO publications_d16_new (
  id,
  bar_id,
  status,
  operation,
...
SELECT
  ...
  published_at,
  commit_sha,
  CASE WHEN commit_sha IS NOT NULL THEN 'legacy-deployment-' || id ELSE NULL END,
  CASE WHEN status = 'success' THEN 'success' ELSE NULL END,
  commit_sha,
  CASE WHEN commit_sha IS NOT NULL THEN 'https://fake-cloudflare.example.test/deployments/' || id ELSE NULL END,
  CASE WHEN commit_sha IS NOT NULL THEN created_at ELSE NULL END,
  CASE WHEN commit_sha IS NOT NULL THEN completed_at ELSE NULL END,
...
ALTER TABLE publications_d16_new RENAME TO publications;
