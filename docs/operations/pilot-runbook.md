...
- preview: production과 다른 D1 database 이름과 binding을 사용한다.
...
5. 첫 publish 전 `public/menus/{encodedSlug}.json`과 trigger file path를 preview에서 확인한다.
...
3. preview D1에 같은 migration을 적용해 `npm run verify:migrations`와 smoke test를 통과시킨다.
...
2. preview 환경 secret에 먼저 반영한다.
3. preview publish와 deployment 조회를 확인한다.
4. production secret을 교체한다.
5. 기존 token을 폐기한다.
...
2. preview에서 public JSON schema 통과를 확인한다.
